...
async def generate(
    ...
    try:
        ...
        # Emotion sample — от Android если есть, иначе пропускаем
        if emotion is not None:
            emotion_raw = await emotion.read()
            if emotion_raw:
                emotion_filename = emotion.filename or "emotion.ogg"
                emotion_ext = emotion_filename.rsplit(".", 1)[-1].lower() if "." in emotion_filename else "ogg"
                print(f"Emotion sample received: {emotion_filename}, size: {len(emotion_raw)} bytes")
                emotion_wav = convert_to_wav(emotion_raw, emotion_ext)
                references.append(ReferenceAudio(audio=emotion_wav, text=""))
                print(f"Emotion converted to wav: {len(emotion_wav)} bytes")
            else:
                print("Emotion field present but empty — skipping")
        else:
            print("No emotion sample provided — generating with voice only")
